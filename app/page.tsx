...
export default function Home() {
  ...
  const [frequencyAssignment, setFrequencyAssignment] = useState<
  ...
  const removeAntenna = (antenna: string) => {
    ...
    setConstraints(
      constraints.filter(([a1, a2]) => a1 !== antenna && a2 !== antenna)
    );
  };
  ...
  const addConstraint = () => {
    if (antenna1 && antenna2 && antenna1 !== antenna2) {
      const constraintExists = constraints.some(
        ([a1, a2]) =>
          (a1 === antenna1 && a2 === antenna2) ||
          (a1 === antenna2 && a2 === antenna1)
      );
  ...
  return (
    <main className="container mx-7 py-8 ">
      <h1 className="text-3xl font-bold mb-8 text-center">
        Antenna Frequency Allocator
      </h1>
        ...
        <div className="space-y-8">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define which antennas interfere with each other
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-8 w-[50rem]">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Visual representation of antennas and constraints
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="border rounded-md p-4 h-[400px] flex items-center justify-center">
                {antennas.length > 0 ? (
                  ...
                  <p className="text-muted-foreground">
                    Add antennas to visualize the network
                  </p>
                )}
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Assigned frequencies for each antenna
              </CardDescription>
            </CardHeader>
            <CardContent>
              {Object.keys(frequencyAssignment).length > 0 ? (
                <>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    {Object.entries(frequencyAssignment).map(
                      ([antenna, frequency]) => (
                        <div
                          key={antenna}
                        ...
                        >
                          <div
                            className={`w-4 h-4 rounded-full ${getColorForFrequency(
                              frequency
                            )}`}
                          ></div>
                          <span>
                  ...
                  <div className="mt-4 pt-4 border-t">
                    <p className="font-semibold">
                      Minimum Frequencies Used: {minFrequencies}
                    </p>
                  </div>
                </>
              ) : (
